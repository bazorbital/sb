...
Administrators gain a dedicated **Calendar** view under **Smooth Booking → Calendar** to inspect each location's day-at-a-glance schedule using an EventCalendar-powered day view. The screen defaults to today's bookings, renders employee resources as columns, applies service colours to every appointment, and offers lightweight location and date filters so teams can jump between venues without losing context.

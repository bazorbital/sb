...
- Visit **Smooth Booking → Calendar** to inspect location schedules at a glance, review employee availability per slot, colour-coded appointments by service, filter by services or staff via multi-selects or quick chips, and create/edit/delete bookings directly from the grid using the shared appointment form modal.

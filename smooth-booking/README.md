...
- Visit **Smooth Booking → Calendar** to inspect location schedules at a glance in a day-based EventCalendar view that defaults to today, rendering employee resources as columns, applying service colours to each appointment, and letting you switch locations, employees, services, or dates with lightweight native controls that refetch events via AJAX. Use the **Add appointment** button or drag across the grid to open the booking modal, which auto-fills start/end times from the selected slot and the chosen service duration.
...
- Visit **Smooth Booking → Email notifications** to configure automated emails for booking events, choose recipients (customer, staff, administrators—any combination—or a custom list on its own), limit by appointment status or services, attach ICS files, and compose HTML/Text bodies with placeholder codes. Duplicate event/recipient combinations are detected before saving so you never hit database errors while adding rules.
...
- Use the REST API for automation:
  ...
  - `GET /wp-json/smooth-booking/v1/calendar/schedule` — fetch location/day EventCalendar payloads with optional `location_id`, `date`, and `resource_ids[]` filters, returning resources, events, and slot boundaries.
...
- Enable `window.SmoothBookingCalendarDebug.toggleCustomerTriggerTracing(true)` to log captured click/pointer context for the “Add new customer” button when the inline customer dialog refuses to open alongside the booking modal.

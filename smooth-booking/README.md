...
- Calendar workspace showing per-location employee columns, configurable time-slot length, service-coloured appointments, inline booking modal, and quick edit/delete affordances
...
- Visit **Smooth Booking → Calendar** to inspect location schedules at a glance, review employee availability per slot, colour-coded appointments by service, and create/edit/delete bookings directly from the grid using the shared appointment form modal.
